...
class MapGenerator {
        ...
        while (points.length < count) {
            ...
            while (attempts < maxAttempts && !validPoint) {
                ...
                for (const existing of points) {
                    const distance = Math.sqrt(Math.pow(candidate.x - existing.x, 2) +
            ...
            if (!validPoint)
                break; // Couldn't find more valid points
        }
        return points;
    }
    calculateNeighbors(territories) {
        ...
        for (let i = 0; i < territories.length; i++) {
            ...
            for (let j = 0; j < territories.length; j++) {
                ...
                const distance = Math.sqrt(Math.pow(territory.x - other.x, 2) +
    ...
    distributeUnits(territories, players) {
    ...
    assignTerritories(territories, players) {
